...
import type { OrderMoneyDistributionSnapshot } from "@/lib/types";
    ...
    <div className="rounded-xl border border-brand-100 bg-white">
      ...
      >
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-brand-900">{orderLabel}</p>
          <p className="text-[10px] text-gray-500">Order {orderId.slice(0, 8)}… · {formatGhc(amount)}</p>
        </div>
                    ...
                    {snapshot.lines.map((line) => (
                      <tr key={line.id} className="border-b border-brand-50">
                        <td className="py-2.5 pr-3">
                          <p className="font-semibold text-brand-900">{line.roleLabel}</p>
                          <p className="text-[10px] text-gray-500">{line.recipientName}</p>
                        </td>
